...
from hammer.vlsi import HammerTool, HammerTimingTool, HammerPlaceAndRouteTool, HammerToolStep, HammerToolHookAction, MMMCCornerType, PlacementConstraintType, TCLTool
...
class OpenROADPlaceAndRoute(HammerTimingTool, OpenROADPlaceAndRouteTool):
    ...
    def steps(self) -> List[HammerToolStep]:
        steps = [
            self.init_design, # Self.run_sta
            self.write_design # Self.run_sta
        ]
    ...
    def create_archive(self, output: str, code: int) -> bool:
        ...
        TODOs:
        - Split sta.tcl into constituent steps, conditional filter out everything after floorplan
        ...
        subprocess.call(["sed", "-i", "-E", r"/repair_tie_fanout/! s#(/[^/ ]+)+/([^/ ]+/)*([^/ ]+)#\3#g", os.path.join(issue_dir, "sta.tcl")])
        # Comment out exec klayout block
        klayout_bin = self.get_setting('par.openroad.klayout_bin')
        subprocess.call(["sed", "-i", f"s/\(exec {klayout_bin}\)/# \\1/g", os.path.join(issue_dir, "sta.tcl")])
    ...
    def run_openroad(self) -> bool:
        ...
        # Create par script.
        par_tcl_filename = os.path.join(self.run_dir, "sta.tcl")
    ...
    def init_design(self, first_step=None) -> bool:
        ...
        #
        # sta_script = os.path.join(self.run_dir, "sta.tcl")
        #
    ...
    def create_write_reports_tcl(self) -> List[str]:
        ...
        self.tcl_append(f"write_reports {self.top_module}\n", write_reports_cmds)
